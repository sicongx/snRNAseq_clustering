...
save_filter_qc_plots <- function(md, th, out_dir, stage, seurat_filt = NULL, n_before = NULL, md_before = NULL) {
  plot_dir <- file.path(out_dir, "qc_filter_plots", threshold_folder_label(th), stage)
  dir.create(plot_dir, recursive = TRUE, showWarnings = FALSE)
  md$orig.ident <- factor(md$orig.ident, levels = sort(unique(md$orig.ident)))

  show_feature_lines <- is.null(th$mode) || !identical(th$mode, "quartile")
  nF_min <- if (show_feature_lines) th$nFeature_min else NULL
  nF_max <- if (show_feature_lines) th$nFeature_max else NULL

  save_gg(
    plot_qc_density(md, "nFeature_RNA", paste(stage, "nFeature density"), "nFeature_RNA", nF_min, nF_max),
    file.path(plot_dir, "density_nFeature"), width = 14, height = 10
  )
  save_gg(
    plot_qc_density(md, "nCount_RNA", paste(stage, "nCount density"), "nCount_RNA", NULL, NULL),
    file.path(plot_dir, "density_nCount"), width = 14, height = 10
  )
  save_filter_zoom_box_plots(md, th, plot_dir, stage, y_min = 200, y_max = 3000)
  save_gg(
